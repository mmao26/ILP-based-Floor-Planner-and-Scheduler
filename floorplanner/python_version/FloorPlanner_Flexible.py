...
WIDTH_REC = int((area_estimate/0.6)**0.5)
Enlarge_Fac = 20.0
...
for i in range(NUM_RESOURCES-1):
    for j in range(i+1, NUM_RESOURCES):
        mdl.add((vx[i] + (1-vz[i])*SIZE_WIDTH[i] + vz[i]*SIZE_HEIGHT[i] <= vx[j] + max(WIDTH_REC,HEIGHT_REC)*(vxy[i][j]+vxy[j][i])))
for i in range(NUM_RESOURCES-1):
    for j in range(i+1, NUM_RESOURCES):
        mdl.add((vx[i] - (1-vz[j])*SIZE_WIDTH[j] - vz[j]*SIZE_HEIGHT[j] >= vx[j] - max(WIDTH_REC,HEIGHT_REC)*(1+vxy[i][j]-vxy[j][i])))
for i in range(NUM_RESOURCES-1):
    for j in range(i+1, NUM_RESOURCES):
        mdl.add((vy[i] + (1-vz[i])*SIZE_HEIGHT[i] + vz[i]*SIZE_WIDTH[i] <= vy[j] + max(WIDTH_REC,HEIGHT_REC)*(1-vxy[i][j]+vxy[j][i])))
for i in range(NUM_RESOURCES-1):
    for j in range(i+1, NUM_RESOURCES):
        mdl.add((vy[i] - (1-vz[j])*SIZE_HEIGHT[j] - vz[j]*SIZE_WIDTH[j] >= vy[j] - max(WIDTH_REC,HEIGHT_REC)*(2-vxy[i][j]-vxy[j][i])))
...
if msol and visu.is_visu_enabled():
    ...
    H_val = msol.get_var_solution(H)
    TOTAL_HEIGHT = H_val.get_value()
    print("Total Area is {}".format(TOTAL_HEIGHT*WIDTH_REC/Enlarge_Fac/Enlarge_Fac))
    for i in range(NUM_RESOURCES):
        # Display square i
        sx, sy, sz = msol.get_var_solution(vx[i]), msol.get_var_solution(vy[i]), msol.get_var_solution(vz[i])

        # transform (rotation)
        if sz.get_value():
            sx1, sx2, sy1, sy2 = sx.get_value(), sx.get_value()+SIZE_HEIGHT[i], sy.get_value(), sy.get_value()+SIZE_WIDTH[i]
        else:
            sx1, sx2, sy1, sy2 = sx.get_value(), sx.get_value()+SIZE_WIDTH[i], sy.get_value(), sy.get_value()+SIZE_HEIGHT[i]
